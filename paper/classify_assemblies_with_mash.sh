...
mash sketch -o all_sketches -s 100000 -p 32 */*/*.fna.gz
...
for sample in $(echo $sample_names); do
    ...
    sbatch -p sysgen --nodes=1 --job-name=mash_assembly --ntasks=1 --cpus-per-task=4 --mem=12000 --time=0-1:0:00 --wrap "/usr/bin/time -v "$BACSORT"/scripts/classify_using_mash.py "$sketch" "$assembly
    sleep 0.1
done
