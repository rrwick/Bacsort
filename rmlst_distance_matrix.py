...
def main():
    ...
    for assembly in assembly_files:
        ...
        print('\nSearching for genes in {}'.format(assembly))
        gene_seqs[assembly] = {}

        if args.threads == 1:
            for gene in gene_files:
                find_gene_seq_for_assembly(assembly, db_name, gene, gene_seqs, assembly_seqs, args)
        else:
            pool = ThreadPool(args.threads)
            pool.map(lambda gene: find_gene_seq_for_assembly(assembly, db_name, gene, gene_seqs,
